...
7. **数据只走 `Store`**（`js/storage.js`）；日期一律用 `Store.today()/addDays()` 的本地时区 `YYYY-MM-DD`，禁用 `toISOString()` 派生日期。
8. **转义纪律**：用户输入插入 HTML 前必须过 `app.js` 内的 `esc()`。
9. 用户未要求时不做 git 提交/推送（注意：上级目录的 `.git` 是空目录，非有效仓库）。
