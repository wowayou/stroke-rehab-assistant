...
- 训练示范是文字分步，无图片/视频/语音
- **训练/用药/游戏历史无应用内视图**（真实缺口，非设计决策）：训练打卡只有"今日已完成"标记与连续天数汇总，无"哪天练了哪些"的历史；用药核对只有今日表 + 近 7 天依从率%，无逐日历史；游戏成绩只记录不展示。导出报告也只给汇总
- **记录页历史列表位于输入表单下方**，老人小屏手机上容易漏看（2026-08-02 用户真机实测反馈："以为记录只有导出才能看到"）；是否将"查看记录"前置/优先于输入，待决策
