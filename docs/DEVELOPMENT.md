...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-08-03 | v0.2.9.1 | 运维：`deploy.sh` 不再硬依赖全局 wrangler（找不到则回落 `npx wrangler@latest`），并在部署前检查登录状态、未登录时给出明确指引后退出。起因：本机 wrangler 未全局安装且 `wrangler whoami` 报未登录，v0.2.9 因此**只推送了代码、未上线**（HANDOVER §1.2 已注明）。无应用代码改动。 |
